  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <nav className="border-b border-[#E2E8F0] px-6 py-4 flex items-center justify-between max-w-6xl mx-auto">
        ...
        <Link href="/admin/courses/new" className="bg-[#2563EB] text-black px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#2563EB]/90 transition-colors">
          + Thêm khóa học
        </Link>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        {courses.length === 0 ? (
          <div className="bg-slate-50 border border-[#E2E8F0] rounded-xl p-16 text-center text-[#64748B]">
                ...
                {courses.map(c => (
                  <tr key={c.id} className="border-b border-[#E2E8F0] last:border-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${c.published ? 'bg-green-500/20 text-green-400' : 'bg-white text-[#64748B]'}`}>
                        {c.published ? 'Đã đăng' : 'Nháp'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <Link href={`/admin/courses/${c.id}`} className="text-[#2563EB] text-sm hover:underline">Sửa</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
