  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <nav className="border-b border-slate-200 px-6 py-4 flex items-center justify-between max-w-4xl mx-auto">
        <Link href="/courses" className="text-slate-400 text-sm hover:text-white transition-colors">← Khóa học</Link>
            ...
            return (
              <Link key={lesson.id} href={`/practice/${lesson.id}`}
                className={`flex items-center justify-between p-5 rounded-xl border transition-all group ${
                  done ? 'border-green-500/20 bg-green-500/5' : 'border-slate-200 bg-slate-50 hover:border-indigo-200'
                }`}>
                <div className="flex items-center gap-4">
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${
                    done ? 'bg-green-500/20 text-green-400' : 'bg-white text-slate-400'
                  }`}>
                    {done ? '✓' : i + 1}
                  </span>
                  <div>
                    <div className="font-medium group-hover:text-indigo-600 transition-colors">{lesson.title}</div>
                    <div className="text-slate-400 text-xs mt-0.5">{lesson._count.exercises} bài tập</div>
                  </div>
                </div>
                <span className="text-slate-300 text-sm group-hover:text-indigo-600 transition-colors">→</span>
              </Link>
            )
          })}
        </div>
