...
export async function POST() {
  ...
  response.cookies.set('access_token', newAccessToken, { httpOnly: true, secure: process.env.NEXTAUTH_URL?.startsWith("https") ?? false, sameSite: 'lax', maxAge: 365 * 24 * 60 * 60, path: '/' })
  response.cookies.set('refresh_token', newRefreshToken, { httpOnly: true, secure: process.env.NEXTAUTH_URL?.startsWith("https") ?? false, sameSite: 'lax', maxAge: 365 * 24 * 60 * 60, path: '/' })
  return response
}
